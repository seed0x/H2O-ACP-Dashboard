"""
Corrected job import - maps communities to correct builders based on user requirements:
- Toll Brothers: northside, lacamas hills
- Dr. Horton: CC (curtain creek), SCT (south cascade terrace), and other subdivisions
- Timbercrest: custom home builds
- Songbird: Luden Estates, Gardens
"""
import csv
import re
import sys
...
def parse_address(location):
    ...
    city = 'Vancouver'  # Default to Vancouver as per user requirements
    ...
    if len(parts) >= 2:
        ...
        if city == 'Vancouver' and potential_city:
            city = potential_city
    else:
    ...
    return address or '', city or 'Vancouver', 'WA', zip_code or '00000'
...
def map_community_to_builder(community):
    """Map community/subdivision to correct builder based on user requirements"""
    if not community:
        return None
    
    community_upper = community.upper().strip()
    
    # Toll Brothers communities
    if 'NORTHSIDE' in community_upper or community_upper == 'NORTHSIDE':
        return 'Toll Brothers'
    if 'LACAMAS' in community_upper or 'LACAMAS HILLS' in community_upper:
        return 'Toll Brothers'
    
    # Dr. Horton communities
    if 'CURTAIN CREEK' in community_upper or 'CC' in community_upper or community_upper.startswith('CC '):
        return 'Dr Horton'
    if 'SOUTH CASCADE' in community_upper or 'SCT' in community_upper or community_upper.startswith('SCT '):
        return 'Dr Horton'
    if 'CURTAIN' in community_upper and 'CREEK' in community_upper:
        return 'Dr Horton'
    
    # Songbird communities
    if 'LUDEN' in community_upper or 'LUDEN ESTATES' in community_upper:
        return 'Songbird'
    if 'GARDENS' in community_upper and 'SONGBIRD' not in community_upper:  # Gardens but not Songbird Gardens (which is also Songbird)
        return 'Songbird'
    if 'SONGBIRD GARDENS' in community_upper:
        return 'Songbird'
    if 'HUMMINGBIRD' in community_upper:
        return 'Songbird'
    if 'GOLDFINCH' in community_upper:
        return 'Songbird'
    
    # Timbercrest - custom homes (typically no specific subdivision, or custom build indicators)
    if 'TIMBERCREST' in community_upper or 'CUSTOM' in community_upper:
        return 'Timbercrest'
    
    # Other known mappings
    if 'MEADOW' in community_upper or 'MEADOWVIEW' in community_upper:
        return 'Pulte'
    if 'PARKERS LANDING' in community_upper or 'PARKERS' in community_upper:
        return 'Urban Nw'
    if 'NORTH HAVEN' in community_upper:
        return 'Urban Nw'
    if 'IRON HORSE' in community_upper:
        return 'Wharton'
    if 'KAYS PLACE' in community_upper or 'JENNELES' in community_upper:
        return 'Nutter'
    
    return None

def get_builder_from_subject_and_community(builder_from_subject, community):
    """Determine correct builder from subject and community"""
    # First try community mapping (most reliable)
    builder_from_community = map_community_to_builder(community)
    if builder_from_community:
        return builder_from_community
    
    # Fall back to normalized builder from subject
    if builder_from_subject:
        normalized = normalize_builder_name(builder_from_subject)
        return normalized
...
with open(csv_path, 'r', encoding='utf-8-sig') as f:
    ...
    for row in reader:
        ...
        if 'PUNCH' in categories.upper() or 'PUNCH' in subject.upper():
            continue
        builder_from_subject, subdivision, phase, lot_number = parse_subject(subject)
        if not subdivision or not lot_number or not phase:
            continue
        
        # Determine correct builder from community mapping
        correct_builder = get_builder_from_subject_and_community(builder_from_subject, subdivision)
        if not correct_builder:
            print(f"  WARNING: Could not determine builder for community '{subdivision}', skipping", file=sys.stderr)
            continue
        ...
        # Use correct builder for key
        job_key = (correct_builder, subdivision, lot_number)
        
        jobs_by_lot[job_key].append({
            'builder_name': correct_builder,
...
for batch_start in range(0, len(jobs_to_import), batch_size):
    ...
    for job_data in batch:
        normalized_builder = job_data['builder_name']
        builder_id = builder_ids.get(normalized_builder)
        if not builder_id:
            print(f"  -- WARNING: Builder ID not found for '{normalized_builder}', skipping", file=sys.stderr)
            continue
        ...
        lot_num = job_data.get('lot_number', '')
        value = f"  ('all_county', '{builder_id}'::uuid, {escape_sql(job_data['community'])}, {escape_sql(lot_num)}, {escape_sql(job_data['phase'])}, {escape_sql(job_data['status'])}, {escape_sql(job_data['address'])}, {escape_sql(job_data['city'])}, {escape_sql(job_data['state'])}, {escape_sql(job_data['zip'])}, {scheduled_start_sql}::timestamptz, {scheduled_end_sql}::timestamptz, {notes_sql}, {completion_date_sql}::date, {warranty_start_sql}::date, {warranty_end_sql}::date, now(), now())"
        values.append(value)
