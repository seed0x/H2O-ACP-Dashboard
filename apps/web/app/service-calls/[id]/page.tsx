...
export default function ServiceCallDetail({ params }: { params: Promise<{ id: string }> | { id: string } }){
  ...
  useEffect(()=>{
    ...
    async function load(){
      try {
        ...
        const res = await axios.get(`${API_BASE_URL}/service-calls/${id}`, { headers, withCredentials: true })
        setSc(res.data)
        setNotes(res.data.notes || '')
        await loadAudit()
      } catch (err: any) {
        console.error('Failed to load service call:', err)
        setError(err.response?.data?.detail || 'Failed to load service call')
      } finally {
        setLoading(false)
      }
    }
  ...
  async function patch(){
    ...
    try {
      ...
      await axios.patch(`${API_BASE_URL}/service-calls/${id}`, { notes }, { headers, withCredentials: true })
      const res = await axios.get(`${API_BASE_URL}/service-calls/${id}`, { headers, withCredentials: true })
      setSc(res.data)
    } catch (err: any) {
      console.error('Failed to save service call:', err)
      setError(err.response?.data?.detail || err.message || 'Failed to save service call')
    } finally {
      setSaving(false)
    }
  }
  ...
  if(error && !sc) return <div style={{ padding: '32px', color: '#EF5350' }}>Error: {error}</div>
  if(!sc) return <div style={{ padding: '32px', color: 'var(--color-text-secondary)' }}>Service call not found</div>
  ...
  return (
    <main className="p-6">
      ...
      <div className="mt-4">
        ...
        {sc.phone && <a href={`tel:${sc.phone}`} style={{ display: 'block', color: 'var(--color-primary)', marginBottom: '8px' }}>Call: {sc.phone}</a>}
        {sc.email && <a href={`mailto:${sc.email}`} style={{ display: 'block', color: 'var(--color-primary)', marginBottom: '8px' }}>Email: {sc.email}</a>}
        {sc.address_line1 && sc.city && (
          <a href={`https://www.google.com/maps/search/${encodeURIComponent(sc.address_line1 + ' ' + sc.city)}`} target="_blank" rel="noreferrer" style={{ display: 'block', color: 'var(--color-primary)' }}>Open in Google Maps</a>
        )}
      </div>
      <div className="mt-4">
